...
namespace KKIHUB.ContentSync.Web.Controllers
{
	...
	public class ContentController : Controller
	{
		...
		public async Task<ActionResult> SyncContentUpdated(PullModel pullModel)
		{
			...
			var result = CleanUpAndOrderList(content);

			return Json(result, JsonRequestBehavior.AllowGet);
		}


		private IList<ContentModel> CleanUpAndOrderList(IEnumerable<ContentModel> list)
		{
			if (list == null) return null;

			var uniqueItems = new List<ContentModel>();

			foreach (var c in list)
			{
				if (!uniqueItems.Any(x => x.ItemId == c.ItemId))
				{
					uniqueItems.Add(c);
				}
			}
			var maxDepth = uniqueItems.Max(x => x.Level);

			var result = new List<ContentModel>();

			for (var i = 0; i <= maxDepth; i++)
			{
				var levelItems = uniqueItems.Where(x => x.Level == i);

				foreach (var levelItem in levelItems)
				{
					if (levelItem.Parent != null)
					{
						var parentIndex = result.IndexOf(levelItem.Parent);

						result.Insert(parentIndex + 1, levelItem);
					}
					else
					{
